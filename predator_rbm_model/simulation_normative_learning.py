# Simulation of normative learning, used in plotting normative learning example in figure 1
...
pe = np.linspace(0, n_range/2, n_range)
alpha = np.full(n_range, np.nan)
b_t = np.full(n_range, np.nan)
bt_val = np.full(n_range, np.nan)
...
if save_csv:
    # save as csv
    df_norm.to_csv('../Data/simulation_normative_learning.csv',index=False)

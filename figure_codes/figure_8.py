...
for i, perf_block in enumerate(performance_blocks):
    for j, category in enumerate(categories):
        # Filter the data
        data_bp = df_long[(df_long['PerformanceBlock'] == perf_block) & (df_long['G_Category'] == category)][
            'Performance']
        ...
        ax1.scatter(jittered_x, data_bp, s=10, color=colors_bp[category], marker='o', alpha=0.3, facecolor=None,
                    edgecolors='none')

# Calculate test statistics (Welch's t test between low and high internalizing)
# Calculate p-values
t_stable, p_stable, dof_stable, n_lowG, n_highG = compute_test_statistic(df_performance, 'G_Category', 'Performance_B0',
                                                                         'Low', 'High', test='ttest_ind')
t_volatile, p_volatile, dof_volatile, _, _ = compute_test_statistic(df_performance, 'G_Category', 'Performance_B1',
                                                                    'Low', 'High', test='ttest_ind')

# set the title, axes properties and legend
title = "$\it{p_{\mathrm{stable}}}$ = " + str(round(p_stable, 2)) + ", $\it{p_{\mathrm{volatile}}}$ = " + str(
    round(p_volatile, 2))
...
def compute_performance_stats(arr):
    ...
    """
    Compute mean, standard deviation, median, and interquartile range (IQR) for a given array, ignoring NaN values.

    Parameters
    ----------
    arr : numpy.ndarray or pandas.Series
        Input array or series for which the statistics are computed.
...
mean_stable_LowG, std_stable_LowG, median_stable_LowG, stable_LowG_IQI = compute_performance_stats(
    df_performance[df_performance['G_Category'] == 'Low']['Performance_B0'])
mean_stable_HighG, std_stable_HighG, median_stable_HighG, stable_HighG_IQI = compute_performance_stats(
    df_performance[df_performance['G_Category'] == 'High']['Performance_B0'])

# volatile phase
mean_volatile_LowG, std_volatile_LowG, median_volatile_LowG, volatile_LowG_IQI = compute_performance_stats(
    df_performance[df_performance['G_Category'] == 'Low']['Performance_B1'])
mean_volatile_HighG, std_volatile_HighG, median_volatile_HighG, volatile_HighG_IQI = compute_performance_stats(
    df_performance[df_performance['G_Category'] == 'High']['Performance_B1'])

stats_PCorrect = {
    'Statistic': ['mean_stable_lowG', 'std_stable_lowG', 'median_stable_lowG', 'stable_lowG_25', 'stable_lowG_75',
                  'mean_stable_highG', 'std_stable_highG', 'median_stable_highG', 'stable_highG_25', 'stable_highG_75',
                  'stable_t', 'stable_p', 'stable_dof',
                  'mean_volatile_lowG', 'std_volatile_lowG', 'median_volatile_lowG', 'volatile_lowG_25',
                  'volatile_lowG_75',
                  'mean_volatile_highG', 'std_volatile_highG', 'median_volatile_highG', 'volatile_highG_25',
                  'volatile_highG_75',
    ...
    'Values': [mean_stable_LowG, std_stable_LowG, median_stable_LowG, stable_LowG_IQI[0], stable_LowG_IQI[1],
               ...
               mean_volatile_HighG, std_volatile_HighG, median_volatile_HighG, volatile_HighG_IQI[0],
               volatile_HighG_IQI[1],
               round(t_volatile, 2), round(p_volatile, 2), dof_volatile,
               int(len(df_merged)), n_lowG, n_highG, round(np.mean(df_merged['g']), 2),
               round(np.max(df_merged['g']), 2), round(np.min(df_merged['g']), 2)]
    }

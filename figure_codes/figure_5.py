...
ax0.plot(df_normative['PE'], df_normative['PE'] * df_normative['alpha_i'], '-', color=colors[1], linewidth=3,
         alpha=0.85, label='Adaptive LR')
...
r1_reg, p1_reg = plot_x_vs_y_FactorScores_robust(ml_data, 'g', 'beta_1', ax2, title=False, fontsize=fontsize,
                                                 color_index=-2, line_color_index=-1, xlabel='General Factor',
                                                 ylabel='Fixed LR')

title_b1 = "$\it{r}$ = " + str(
    np.round(df_fdr[df_fdr['Statistic'] == 'r_b1_g']['Values'].values[0], 2)) + ", $\it{p}$ = " + str(
    np.round(df_fdr[df_fdr['Statistic'] == 'p_b1_g']['Values'].values[0], 2))
...
r4_reg, p4_reg = plot_x_vs_y_FactorScores_robust(ml_data, 'g', 'beta_4', ax4, title=False, fontsize=fontsize,
                                                 color_index=-2, line_color_index=-1, xlabel='General Factor',
                                                 ylabel='Adaptive LR')

title_b4 = "$\it{r}$ = " + str(
    np.round(df_fdr[df_fdr['Statistic'] == 'r_b4_g']['Values'].values[0], 2)) + ", $\it{p}$ = " + str(
    np.round(df_fdr[df_fdr['Statistic'] == 'p_b4_g']['Values'].values[0], 2))
...
stats = {
    'Statistic': ['t_b1', 'p_b1', 'n_Low', 'n_High', 'dof_b1', 'median_b1_Low', 'median_b1_Low_IQIlow',
                  'median_b1_Low_IQIhigh', 'median_b1_High', 'median_b1_High_IQIlow', 'median_b1_High_IQIhigh',
                  't_b4', 'p_b4', 'dof_b4', 'median_b4_Low', 'median_b4_Low_IQIlow', 'median_b4_Low_IQIhigh',
                  'median_b4_High', 'median_b4_High_IQIlow', 'median_b4_High_IQIhigh'],

    'Values': [t_b1, round(p_b1, 2), nLow, nHigh, dof_b1, median_b1_Low, median_b1_Low_iqr[0], median_b1_Low_iqr[1],
               median_b1_High, median_b1_High_iqr[0], median_b1_High_iqr[1],
               t_b4, round(p_b4, 2), dof_b4, median_b4_Low, median_b4_Low_iqr[0], median_b4_Low_iqr[1], median_b4_High,
               median_b4_High_iqr[0], median_b4_High_iqr[1]]
}

# Plot a figure for the control study with less instructions which shows descriptive as well as model based results
...
ml_data = pd.read_csv(os.path.join(target_dir, 'supplementary_data/predator_task/df_model_exp_LI.csv'))
df_LI = pd.read_csv(os.path.join(target_dir, 'supplementary_data/predator_task/df_behav_exp_LI.csv'))

qns_totalscore = pd.read_csv(os.path.join(target_dir, 'supplementary_data/factor_analysis/qnsdata_exp_LI.csv'),sep=';')
...
# keep only subjects in df_LI that are also in df_qns
df_LI = df_LI[df_LI['subjectID'].isin(pd.unique(df_merged['subjectID']))]
...
stats_LI = {'Statistic': ['mean_EE', 'std_EE', 'mean_LR', 'std_LR',
...
df_stats_model_overall = pd.DataFrame(stats_LI)
...
df_stats_model_overall.name = 'Supplementary_LI_results'

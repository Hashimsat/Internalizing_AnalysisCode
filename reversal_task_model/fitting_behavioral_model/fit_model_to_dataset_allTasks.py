...
def fit_model(args,params, data):
    ...
    # Initialize models
    # ------------
    if ((args.task_type=='MagVersion') or (args.task_type=='MagVersionBoth')):
        if args.modelname=='1':
        ...
        if any(digit in args.modelname for digit in ['0','1', '2', '3', '4', '5', '6', '11']):
            import models_2thr9_11_ReducedTask as model_specific

        if any(digit in args.modelname for digit in ['9','12']):
            import models_9_12_reduced as model_specific
    ...
    if args.exp==2:  # Dataset with reward magnitudes
        includes_subjs_with_one_task = args.one_task_only
    ...
    with model:
        ...
        print('sampling ...')
        trace = pm.sample(args.steps,step=step,chains=2,tune=args.steps_tune,random_seed=args.seed) # cores = 4

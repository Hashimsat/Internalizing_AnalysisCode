# Run parameter recovery on the different hierarchical models
...
def fit_model(args,params, data, Theta_gen):
    ...
    gen_choice, gen_outcome_valence, *_ = f(Theta_gen)

    # create a dictionary with the generated data to fit the model to
    Y_gen = {}
    Y_gen['participants_choice'] = gen_choice
    X_gen = copy.deepcopy(X)
    X_gen['outcome_valence'] = gen_outcome_valence
    ...
    model = create_model_base(X_gen,Y_gen,C, # Changed here
                              params=params,
                              K=len(params),
    ...
    model = model_specific.combined_prior_model_to_choice_model(X_gen,Y_gen,
                                                                param_names=params,
                                                                model=model,
    ...
    filename='ParamRec_' + 'prl_' +args.task_type +'_model='+args.modelname+'try_one_task_'+str(args.one_task_only)+'_covariate='+args.covariate+'_date='+str(now.year)+ \
    ...
    with model:
        ...
        print('sampling ...')
        trace = pm.sample(args.steps,step=step,chains=args.num_chains,tune=args.steps_tune,random_seed=args.seed) # cores = 4
    ...
    with open('./model_fits/'+filename+'.pkl', "wb" ) as buff:
        pickle.dump({'model': model,'trace':trace,'ppc':ppc,'MAP':MAP, 'subj_indices':subj_indices,
                     'Theta_gen':Theta_gen, 'Y_gen':Y_gen, 'X_gen':X_gen
                     }, buff)

...
def create_hyper_prior_group(Nboth,Nrew,Npain,K,rew_slice,pain_slice,mean=0,std=8):
...
def add_covariate_to_hyper_prior(u_both_total,
    ...
    stems_triple = {"Bi3itemCDM": ['Bi1item_w_j_scaled','Bi2item_w_j_scaled','Bi3item_w_j_scaled'],
                    ...
                    "Qns_score": ['STICSA_scaled','BDI_scaled','STAI_scaled'],
             }
...
def create_model_base(X, # observed stuff
    ...
    with model:
        ...
        if hierarchical:
            ...
            # Shape = N subjects with both X K parameters
            if (task_type=='MagVersionBoth'):
                # Use non-centered thetas to overcome funnel effect in sampling, which is more evident for this task
                Theta_both_offset = pm.MvNormal('Theta_both_offset', mu=0, shape=(Nboth, K), cov=T.eye(K))
                Theta_both = pm.Deterministic('Theta_both', u_both_total + T.dot(Theta_both_offset, Sigma))

...
def trial_step_reduced_task(info_A_tm1,info_A_t, # externally provided to function on each trial
            ...
            estimate_tm1,
            choice_kernel_tm1,
            lr_tm1,lr_c_tm1,Binv_tm1,Bc_tm1,eps_tm1,
    ...
    # Value
    choice_val_t = Binv_t*(pdiff_t) + Bc_t*cdiff_t

    # Value
    # choice_val_t = Binv_t*(estimate_tm1) + Bc_t*choice_kernel_tm1
    ...
    return([choice_t,outcome_valence_t,prob_choice_t,choice_val_t,estimate_t,choice_kernel_t,lr_t,lr_c_t,Binv_t,Bc_t,eps_t])
...
def create_choice_model(X,Y,param_names,Theta,gen_indicator=0,B_max=10.0,nonlinear_indicator=0):
    ...
    Binv_baseline = 8; Binv_goodbad = 0; Binv_stabvol = 0; Binv_rewpain = 0
    ...
    lr,lr_c,Binv,Bc,eps), updates = theano.scan(fn=trial_step_reduced_task,
                                            outputs_info=[starting_choice,starting_outcome_valence, # observables
                                                ...
                                                starting_lr,starting_lr_c,starting_Binv,starting_Bc,starting_eps], # note that outcome c flipped are outcome A is assigned good outcome; should be called info
    ...
    return((choice,outcome_valence,prob_choice,choice_val,estimate_r,choice_kernel,lr,lr_c,Binv,Bc,eps), updates)
...
def combined_prior_model_to_choice_model(X,Y,param_names,model,
    ...
    with model:
        ...
        estimate_r,choice_kernel,lr,lr_c,Binv,Bc,eps), updates = create_choice_model(X,Y,param_names,model.Theta,gen_indicator=0,B_max=B_max,nonlinear_indicator=nonlinear_indicator)
...
def create_gen_choice_model(X,Y,param_names,B_max=10.0,seed=1,nonlinear_indicator=0):
    ...
    (choice,outcome_valence,
    prob_choice,choice_val,
    estimate_r,choice_kernel,lr,lr_c,Binv,Bc,eps), updates = create_choice_model(X,Y,param_names,Theta,gen_indicator=1,B_max=B_max,nonlinear_indicator=nonlinear_indicator)
    ...
    [choice,outcome_valence,
    prob_choice,choice_val,
    estimate_r,choice_kernel,lr,lr_c,Binv,Bc,eps],updates=updates)#no_default_updates=True)

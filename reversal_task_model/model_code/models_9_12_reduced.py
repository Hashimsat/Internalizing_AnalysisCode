...
def trial_step(info_tm1_A,info_t_A, # externally provided to function on each trial
            ...
            estimate_tm1_B,
            choice_kernel_tm1,
            lr_tm1,lr_c_tm1,Amix_tm1,Binv_tm1,Bc_tm1,decay_tm1,eps_tm1,
    ...
    return([choice_t,outcome_valence_t,prob_choice_t,choice_val_t,estimate_t_A,estimate_t_B,choice_kernel_t,lr_t,lr_c_t,Amix_t,Binv_t,Bc_t,decay_t,eps_t])
...
def create_choice_model(X,Y,param_names,Theta,gen_indicator=0,B_max=10.0,nonlinear_indicator=0):
    ...
    choice_kernel,
    lr,lr_c,Amix,Binv,Bc,decay,eps), updates = theano.scan(fn=trial_step,
                                            outputs_info=[starting_choice,starting_outcome_valence, # observables
                                                ...
                                                starting_lr,starting_lr_c,starting_Amix,starting_Binv,starting_Bc,starting_decay,starting_eps], # note that outcome c flipped are outcome A is assigned good outcome; should be called info
    ...
    return((choice,outcome_valence,prob_choice,choice_val,estimate_r_A,estimate_r_B,choice_kernel,lr,lr_c,Amix,Binv,Bc,decay,eps), updates)
...
def combined_prior_model_to_choice_model(X,Y,param_names,model,
    ...
    with model:
        ...
        estimate_r_A,estimate_r_B,choice_kernel,lr,lr_c,Amix,Binv,Bc,decay,eps), updates = create_choice_model(X,Y,param_names,model.Theta,gen_indicator=0,B_max=B_max)
...
def create_gen_choice_model(X,Y,param_names,B_max=10.0,seed=1,nonlinear_indicator=0):
    ...
    (choice,outcome_valence,
    prob_choice,choice_val,
    estimate_r_A,estimate_t_B,choice_kernel,lr,lr_c,Amix,Binv,Bc,decay,eps), updates = create_choice_model(X,Y,param_names,Theta,gen_indicator=1,B_max=B_max)
    ...
    [choice,outcome_valence,
    prob_choice,choice_val,
    estimate_r_A,estimate_t_B,choice_kernel,lr,lr_c,Amix,Binv,Bc,decay,eps],updates=updates)#no_default_updates=True)
